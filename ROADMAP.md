...
3. ✅ **Минимальная модель Pilot / Mech / Part.** — сделано (2026-07-08).
   ...
   - `Enemy` на этой итерации `Mech` не получает, остаётся на плоских
     ...
     местах создания в [src/dungeon.py](src/dungeon.py), для единообразия
     с переименованным полем).
...
4. ✅ **Каталог деталей (пока в коде, не в БД).** — сделано (2026-07-08).
   - [src/parts_catalog.py](src/parts_catalog.py) (по аналогии с
     ...
     добавляется к урону ближнего оружия — стартовый "Кортик" (damage 3)
...
6. **Ручные карты вместо процедурной генерации.**
   ...
   - `Dungeon._procedural_generate` и связанный код (`Room`,
     ...
     переименованию `Dungeon` → `Arena`/`Battlefield` (см. п. 7). Не трогаем
...
2. Разделить слот `arms` на два (левая/правая рука) — отложено с Этапа 1
...
3. Привязать оружие (`Inventory.weapons`) к рукам — отложено с Этапа 1
   (2026-07-08): каждая рука как слот под одно оружие, количество оружия
   ограничено количеством рук. Естественно тянет за собой п. 2 (нужно
   сначала решить, сколько рук у меха).
4. Балансировка дроп-таблиц сундуков/наград — здесь и решится, остаётся
   ли подбор сундука в бою или переезжает целиком в пост-матчевую награду.
5. Тесты на сборку меха и расчёт итоговых статов (юнит-тесты, не только
   `tests/test_lobby.py` уровня HTTP).
6. Ревью `SimpleEnemyAI` — не потребует изменений логики принятия решений,
